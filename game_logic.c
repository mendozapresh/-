...
void perform_turn_calculation(BattleContext *ctx) {
    if (strlen(ctx->current_attacker) == 0) return;
...
void handle_attack_announce(BattleContext *ctx, GameMessage *msg) {
    if (strcmp(msg->attacker, ctx->my_pokemon) == 0) return; // Ignore my own echo
...
void handle_calculation_report(BattleContext *ctx, GameMessage *msg) {
    ...
    if (strcmp(msg->attacker, ctx->my_pokemon) == 0) ctx->opponent_hp = msg->defender_hp_remaining;
    else ctx->my_hp = msg->defender_hp_remaining;

    finalize_turn(ctx);
}

void execute_move_command(BattleContext *ctx, const char *move_name) {
    if (!ctx->is_my_turn || ctx->state != STATE_WAITING_FOR_MOVE) return;
...
void process_incoming_message(BattleContext *ctx, GameMessage *msg) {
    if (strcmp(msg->message_type, "BATTLE_SETUP") == 0) handle_battle_setup(ctx, msg);
    else if (strcmp(msg->message_type, "ATTACK_ANNOUNCE") == 0) handle_attack_announce(ctx, msg);
    else if (strcmp(msg->message_type, "CALCULATION_REPORT") == 0) handle_calculation_report(ctx, msg);
